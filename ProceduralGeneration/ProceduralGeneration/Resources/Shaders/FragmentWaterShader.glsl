...
void main()
{
	...
	vec4 textureSet = texture(diffuse, texCoord0 + vec2(timerFrag, 0.0));
	
	colourExport = vec4(colour0.xyz, 0.5) * textureSet * clamp(dot(vec3(0,1,0), normalize(normal0)), 0.4, 1.0);
}